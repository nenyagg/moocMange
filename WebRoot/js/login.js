...
$(document).ready(function() {
	...
	$('#reg').click(function() {
		...
		if ($('#user').val().length < 4 || $('#user').val().length > 16) {
			...
			$('#userCue').html("<font color='red'><b>x</b></font>");
			return false;
		...
		if ($('#passwd').val().length < pwdmin) {
			$('#passwd').focus();
			$('#userCue').html("<font color='red'><b>x密码长度不能小于 "+ pwdmin + "位</b></font>");
			return false;
		}
		if ($('#passwd2').val() != $('#passwd').val()) {
			$('#passwd2').focus();
			$('#userCue').html("<font color='red'><b>x两次密码输入不一致</b></font>");
			return false;
		}
		...
		if (!mail.test($('#email').val())) {
			...
			$('#userCue').html("<font color='red'><b>x邮箱格式错误</b></font>");return false;
		} else {
			$('#qq').css({
				border: "1px solid #D7D7D7",
				boxShadow: "none"
